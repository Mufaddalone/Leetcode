...
class Solution:
    ...
    def get_next_and_prev_smaller_greater(self, arr):
        n = len(arr)
    ...
    def subArrayRanges(self, nums: List[int]) -> int:
        ...
        next_smaller, prev_smaller, next_greater, prev_greater = self.get_next_and_prev_smaller_greater(nums)
        ...
        for i, num in enumerate(nums):
            ...
            print('current index: ', i, ' current_num: ', num)
            max_contribution = num * ((i - prev_greater[i]) * (next_greater[i] - i))
            print('max_contribution: ', max_contribution)
            min_contribution = num * ((i - prev_smaller[i]) * (next_smaller[i] - i))
            print('min_contribution: ', min_contribution)
            print('tot_sum:', tot_sum, ' adding: ', max_contribution - min_contribution)
